...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Demo Airth\'s timer functionality')
    ...
    if args.command:
        ...
        if response.get("success", False) and response.get(
                "timer_type") in ["countdown", "pomodoro"]:
            try:
                while True:
    ...
    # Default demo mode - show a sequence of commands
        ...
        try:
            ...
            response = airth.respond_to_timer_command(
                "set a timer for 0.5 minutes called Demo Timer")
            print(format_response(response) + "\n")
            ...
            while time.time() - start_time < 32:  # Wait up to 32 seconds
                ...
                if not any(
                    t["timer_type"] == "countdown" for t in status.get(
                        "active_timers",
                        [])):
