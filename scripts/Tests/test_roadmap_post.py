...
parser = argparse.ArgumentParser(
    description="Test WordPress roadmap article posting.")
parser.add_argument(
    "--publish",
    action="store_true",
    help="Publish the article instead of saving as draft")
parser.add_argument(
    "--debug",
    action="store_true",
...
if not env_path.exists():
    logger.error(
        f".env file not found at {env_path}. Please create this file with your credentials.")
    print(
    ...
    try:
        ...
        if result.get("success"):
            ...
            if wp_url and 'wp_response' in result and 'post_id' in result['wp_response']:
                admin_url = f"{wp_url}/wp-admin/post.php?post={
                    result['wp_response']['post_id']}&action=edit"
                print(f"\n📝 View/Edit in WordPress Admin: {admin_url}")

            if post_status == "publish":
                if wp_url and 'wp_response' in result and 'post_id' in result['wp_response']:
                    post_url = f"{wp_url}/?p={
                        result['wp_response']['post_id']}"
                    print(f"🌐 View Published Post: {post_url}")
                else:
                    print("✅ Article published! Visit your WordPress site to view it.")
            else:
                print(
                    "✅ Article saved as draft. Log into WordPress admin to review and publish.")
