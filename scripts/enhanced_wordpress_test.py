...
def test_rest_api_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        if not wp_agent.wp_site_url or not wp_agent.wp_user or not wp_agent.wp_app_pass:
            logger.error(
                "WordPress REST API not configured (missing environment variables)")
            return {
                "success": False,
...
<p>This post was created to verify WordPress integration functionality.</p>"""
    ...
    try:
        ...
        if not wp_xmlrpc.client:
            ...
            if xmlrpc_result.get("success"):
                logger.info(
                    f"XML-RPC draft post created: ID {xmlrpc_result.get('post_id')}")
                results["xmlrpc"] = {
                    "success": True,
                ...
                logger.error(
                    f"Failed to create XML-RPC draft post: {xmlrpc_result.get('error')}")
    ...
    try:
        ...
        if rest_result.get("success"):
            logger.info(
                f"REST API draft post created: ID {
                    rest_result.get('post_id')}")
            results["rest"] = {
                "success": True,
            ...
            logger.error(
                f"Failed to create REST API draft post: {
...
def test_wordpress_utilities() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            optimized = WordPressContentTools.optimize_html_content(
                test_content)
            ...
            images = WordPressContentTools.extract_images_from_content(
                img_content)
...
Examples:
  ...
  python scripts/enhanced_wordpress_test.py --create-post --utilities
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Run all WordPress integration tests")
    parser.add_argument(
        "--connection",
        action="store_true",
        help="Test WordPress basic connection")
    parser.add_argument(
        "--xmlrpc",
        action="store_true",
        help="Test XML-RPC connection specifically")
    parser.add_argument(
        "--rest-api",
        action="store_true",
        help="Test REST API connection specifically")
    parser.add_argument(
        "--create-post",
        action="store_true",
        help="Test creating draft posts")
    parser.add_argument(
        "--utilities",
        action="store_true",
        help="Test WordPress utility functions")
    parser.add_argument(
        "--troubleshooting",
        action="store_true",
