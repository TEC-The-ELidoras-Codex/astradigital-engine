...
try:
    ...
    logger.error(
        "PyGithub library not found. Please install it with 'pip install PyGithub'")
...
def connect_to_github() -> Optional[Any]:
    ...
    if not GITHUB_AVAILABLE:
        logger.error(
            "Cannot connect to GitHub: PyGithub library not available")
        return None
...
def main():
    ...
    create_parser = subparsers.add_parser(
        'create', help='Create a new repository')
    ...
    check_parser = subparsers.add_parser(
        'check', help='Check if a repository exists')
    ...
    if args.command == 'create':
        result = create_repository(
            args.name, args.description or "", args.private)
        if result["success"]:
