...
def main():
    ...
    try:
        ...
        result = airth.create_wordpress_article_about_roadmap(
            roadmap_details, post_status)

        if result.get("success"):
            print(
                f"Successfully created WordPress article: {
                    result.get('title')}")
            print(f"Status: {post_status}")
            if post_status == "draft":
                print(
                    "The article is now in your WordPress drafts. Login to your WordPress admin to review and publish.")
        else:
