        ...
        for line in lines:
            ...
            if wp_section_found:
                if url and line.strip().startswith("WP_URL="):
                    ...
                    new_lines.append(
                        f"WP_APP_PASSWORD_NAME={app_password_name}\n")
                else:
                    new_lines.append(line)

                # If we detect a new section, stop processing WP section
                if line.startswith(
                        "#") and "WordPress Configuration" not in line and not line.strip().startswith("#"):
                    wp_section_found = False
            else:
                new_lines.append(line)
...
def test_wp_connection(wp_url=None, wp_username=None, wp_password=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "administrator" in user_data.get(
                'roles',
                []) or "editor" in user_data.get(
                'roles',
                []) or "author" in user_data.get(
                'roles',
                    []):
                logger.info("✅ User has sufficient publishing permissions")
            else:
                logger.warning(
                    "⚠️ User may not have sufficient publishing permissions")
            ...
            logger.info(
                f"Testing WordPress XML-RPC connection to {xmlrpc_url}")
            ...
            try:
                xmlrpc_response = requests.post(xmlrpc_url, data=json.dumps(
                    xmlrpc_data), headers=xmlrpc_headers, timeout=10)
                if xmlrpc_response.status_code == 200:
                    logger.info("✅ XML-RPC connection successful")
                else:
                    logger.warning(
                        f"⚠️ XML-RPC connection failed with status {xmlrpc_response.status_code}")
                    logger.warning(
                        "This is not critical as we will prefer REST API for publishing")
            ...
            logger.error(
                f"❌ WordPress authentication failed with status {
...
def update_news_automation_configuration(
        max_age=None,
        max_topics=None,
        schedule_time=None):
    ...
    try:
        ...
        logger.info(
            f"Created PowerShell configuration script at {ps_script_path}")
        logger.info(
            f"Run this script as administrator to configure the scheduled task")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Configure WordPress for Airth News Automation")
    ...
    if args.instructions:
        ...
        print(
            "7. Run this script with: --update-wp-config --app-password 'your new password'")
        print("=" * 80)
    ...
    if args.update_automation:
        if update_news_automation_configuration(
                args.max_age, args.max_topics, args.schedule_time):
